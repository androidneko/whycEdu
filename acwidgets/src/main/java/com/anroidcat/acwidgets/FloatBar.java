/**     
* FloatBar.java Create on 2011-5-19    
*     
* Copyright (c) 2011-5-19 by whty  
*     
* @author xjin    
* @version 1.0
*    
...
public class FloatBar extends LinearLayout{
    ...
    /* package */  static final int DEFAULT_SCREEN_WIDTH = 1080;
	...
	public FloatBar(Context context, AttributeSet attrs) {
		super(context, attrs);
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.FloatNaviBar);
		...
		mIsTabArrowEnable = a.getBoolean(R.styleable.FloatNaviBar_tabArrowEnabled, true);
		a.recycle();
	...
	public void fitScreenWidth(int width){
		this.mScreenWidth = this.mScreenWidth-width;
	}
	 ...
	 * @return params
	 */
	private LayoutParams divideChildWidth(int count){
		if (count == mTabCounts || !mIsTabArrowEnable) {
			mChildItemWidth = mScreenWidth/mTabCounts;
			fixedDiff = mScreenWidth % mTabCounts;
		}else {
			mChildItemWidth = (mScreenWidth - mLeftDrawable.getIntrinsicWidth()-mRightDrawable.getIntrinsicWidth()-4)/mTabCounts;
			fixedDiff = (mScreenWidth - mLeftDrawable.getIntrinsicWidth()-mRightDrawable.getIntrinsicWidth()-4) % mTabCounts;
		}
		LayoutParams params = new LayoutParams(mChildItemWidth,LayoutParams.FILL_PARENT);
		params.gravity = Gravity.CENTER_VERTICAL;
		return params;
	}

	public void setOnFloatItemClickListener(OnFloatItemClickListener clickListener){
	...
	protected class MyHorizontalScrollView extends HorizontalScrollView{
		...
		public void setMyHorizontalScrollView(View view,int index,int count){
			view.setOnClickListener(new ItemClick(index,mAdapter.getItem(index)));
			if (index == count -1) {
				LinearLayout.LayoutParams params = divideChildWidth(count);
		...
		protected void onScrollChanged(int l, int t, int oldl, int oldt) {
			...
			int diff = (view.getRight() - (getWidth() + getScrollX()));// Calculate the scroll diff
			if (mIsTabArrowEnable) {
